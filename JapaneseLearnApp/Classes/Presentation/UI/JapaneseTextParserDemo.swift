...
// MARK: - 日语文本解析器示例
struct JapaneseTextParserDemo: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            // 富文本展示区域
            RichTextView(htmlString: sampleJapaneseText) { word in
...
struct RichTextView: UIViewRepresentable {
            ...
            while let match = rubyRegex.firstMatch(in: tempString, options: [], range: NSRange(location: 0, length: tempString.utf16.count)) {
                if let wordRange = Range(match.range(at: 1), in: tempString),
                   ...
                   let fullMatchRange = Range(match.range, in: tempString) {
                    ...
                    // 创建带有注音的富文本
                    let wordAttr = createWordAttributedString(word: word, furigana: furigana)
    ...
    // 创建带有注音的富文本
    private func createWordAttributedString(word: String, furigana: String) -> NSAttributedString {
        ...
        // 添加注音信息（在实际应用中可以使用更复杂的布局）
        let furiganaAttributes: [NSAttributedString.Key: Any] = [
            .font: UIFont.systemFont(ofSize: 10),
            .foregroundColor: UIColor.gray
        ]
        
        let furiganaString = NSAttributedString(string: "(\(furigana))", attributes: furiganaAttributes)
        attributedString.append(furiganaString)
...
// MARK: - 预览
struct JapaneseTextParserDemo_Previews: PreviewProvider {
